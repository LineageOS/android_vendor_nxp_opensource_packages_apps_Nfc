...
tNFA_STATUS JcopManager::JcopInitialize ()
{
    ...
    Pgpx_Jcop_handle = dlopen("vendor/lib64/libp61-jcop-kit.so",RTLD_NOW);
    if(Pgpx_Jcop_handle == NULL){
        ALOGE("%s: Error : opening (vendor/lib64/libp61-jcop-kit.so) !!", __func__);
...
tNFA_STATUS JcopManager::JcopDeInitialize ()
{
    if(Pgpx_Jcop_handle != NULL){
        if((dlclose(Pgpx_Jcop_handle)) != DL_STATUS_OK){
            ALOGE("%s: Error : closing (vendor/lib64/libp61-jcop-kit.so) !!", __func__);
            return NFA_STATUS_FAILED;
        }
    }else{
